	...
	(
		...
		if (variant === "circle") {
			...
			return (
				<div
					ref={ref}
					role="progressbar"
					aria-valuenow={isIndeterminate ? undefined : val}
					aria-valuemin={0}
					aria-valuemax={100}
					aria-valuetext={`${val}%`}
					className={clsx(progressVariants({ color, size, variant }), className)}
					{...props}
				>
		...
		return (
			<div
				ref={ref}
				{...props}
				className={clsx(progressVariants({ color, size }), className)}
				role="progressbar"
				aria-valuenow={isIndeterminate ? undefined : val}
				aria-valuemin={0}
				aria-valuemax={100}
			...
			>
				<div className={styles["lambda-progress-container"]}>
					...
					<div className={styles["lambda-progress-bar-container"]}>
						...
						{showValue && !isIndeterminate && (
							<span aria-label="progress-value" className={clsx(progressValueVariants({ size }))}>
								{displayValue}%
							</span>
						)}
					</div>
