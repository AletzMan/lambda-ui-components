...
import { AlertProps, AlertColor } from './alert.types';
import { alertVariants } from './alert.variants';


// --- Función helper para obtener el icono por defecto según la colore ---
const getDefaultIcon = (color: AlertColor) => {
    switch (color) {
        case 'success':
...
export const Alert = forwardRef<HTMLDivElement, AlertProps>(
    (
        ...
        const renderedIcon = useMemo(() => {
            ...
            if (customIcon && color === "default") {
                // Asegurarse de que el customIcon tenga la clase de estilo si es necesario para el tamaño o color
                if (React.isValidElement(customIcon)) {
                    return React.cloneElement(customIcon as ReactSVGElement, { className: clsx((customIcon.props as React.HTMLAttributes<HTMLDivElement>).className, styles['lambda-alert-icon']) });
                }
        ...
        const alertClasses = clsx(
            styles['lambda-alert'],
            alertVariants({ color, size, variant }),
        ...
        return (
            ...
            >
                ...
                {renderedIcon && (
                    <div className={styles['lambda-alert-icon-container']}>
                        {renderedIcon}
                    </div>
                )}
                <div className={styles['lambda-alert-content']}>
                    ...
                    <p className={styles['lambda-alert-message']}>
                        {message}
                    </p>
                </div>
