...
export const Link = forwardRef<LinkRef, LinkProps>(
    (
        ...
        const linkClasses = clsx(
            ...
            ...(isButtonType ? [
                button({ variant, color, size, radius, loading, disabled }), linkButtonProps({ active, justify })
        ...
        const iconElement = useMemo(() => {
            if (icon && isButtonType) {
                return (
                    <span className={clsx(isButtonType ? buttonStyles['lambda-btn-icon'] : styles['lambda-link-icon'])}>
                        {icon}
                    </span>
                );
            }
        ...
        const contentElement = useMemo(() => {
            if (content) {
                return (
                    <span className={clsx(styles['lambda-link-content'])}>
                        {content}
                    </span>
                );
            }
        ...
        const renderContent = useMemo(() => {
            if (!isButtonType || (!iconElement && !contentElement)) {
                return <>{iconElement}{contentElement}</>;
            }
