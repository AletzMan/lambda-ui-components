...
export const Range = forwardRef<HTMLDivElement, RangeProps>(
    (
        ...
        const getValueFromPointerEvent = useCallback((event: React.PointerEvent<HTMLDivElement>) => {
        ...
        const handleHandlePointerDown = useCallback((event: React.PointerEvent<HTMLDivElement>, handleIndex: number) => {
            if (disabled) {
                return;
            }
        ...
        }, [isDragging, draggingHandleIndex, trackRef, getValueFromPointerEvent, value, isDoubleHandled, onInput, min, max]);
        ...
        const handlePointerUp = useCallback((event: React.PointerEvent<HTMLDivElement>) => {
            ...
            if (isDoubleHandled) {
                ...
                const valueToApply = finalValue === null
                    ? (draggingHandleIndex === 0 ? currentStart : currentEnd)
                ...
                if (draggingHandleIndex === 0) {
                    ...
                    nextValue = [clampedStart, currentEnd];
                } else { // Finalizando arrastre del handle de fin (index 1)
                ...
                const valueToApply = finalValue === null ? value as number : valueToFinalize as number;
        ...
        }, [isDragging, draggingHandleIndex, trackRef, getValueFromPointerEvent, value, isDoubleHandled, onChange, min, max]);
        ...
        const handleKeyDown = useCallback((event: React.KeyboardEvent<HTMLDivElement>, handleIndex: number) => {
            if (disabled) return;
            ...
            if (event.key === 'ArrowLeft' || event.key === 'ArrowDown') {
                ...
                if (isDoubleHandled) {
                    if (handleIndex === 0) delta = (value as [number, number])[1] - (value as [number, number])[0];
            ...
            >
                ...
                >
                    ...
                    <div
                        ...
                        aria-label={isDoubleHandled ? (Array.isArray(ariaLabel) ? ariaLabel[0] : (ariaLabel as string || 'Inicio del rango')) : (ariaLabel as string)}
                        aria-valuemin={min}
                        aria-valuemax={max}
                    ...
                    {isDoubleHandled && (
                        <div
                            ...
                            aria-label={Array.isArray(ariaLabel) ? ariaLabel[1] : `${ariaLabel || 'Fin del rango'}`}
                            aria-valuemin={min}
                            aria-valuemax={max}
