...
export function useNumberInput({
    ...
    const latestValuesRef = useRef({
        ...
        parseValue: ((_input: string | undefined | null) => '') as (input: string | undefined | null) => string,
        formatValue: ((_value: number | undefined | null) => '') as (value: number | undefined | null) => string,
    ...
    useEffect(() => {
        ...
        // console.log("latestValuesRef updated:", latestValuesRef.current);
    }, [isEditing, internalValue, isControlled, controlledValue, min, max, step, onChange]);


    // --- Funciones de Parseo y Formato ---
    const formatValue = useCallback((value: number | undefined | null): string => {
    ...
    useEffect(() => {
        // console.log("useEffect sync: Triggered", { controlledValue, isControlled, isEditing });
        if (isControlled && !isEditing) {
            const valueToFormat = controlledValue !== undefined && controlledValue !== null ? controlledValue : undefined;
            // console.log("useEffect sync: Setting internalValue from controlledValue", valueToFormat);
            setInternalValue(formatValue(valueToFormat));
        } else {
            // console.log("useEffect sync: Not syncing from controlledValue");
        }
    ...
    const handleFocus = useCallback(() => {
        ...
        const rawStringValue = (valueToParse !== undefined && valueToParse !== null && !isNaN(valueToParse)) ? valueToParse.toString() : "";
    ...
    const performDecrementStep = useCallback(() => {
        ...
        const currentNumericValue = !latestIsEditing && latestNumericValue !== undefined && latestNumericValue !== null && !isNaN(latestNumericValue) ? latestNumericValue : Number(latestParseValue(latestInternalValue)) || 0;
    ...
    const startAction = useCallback((action: () => void) => {
        if (disabled) {
            return;
        }
    ...
    const numericValue = useMemo(() => {
        if (isEditing) {
            ...
            const valueToParse = isControlled ? (controlledValue !== undefined && controlledValue !== null ? controlledValue.toString() : "") : internalValue;
    ...
    const formattedDisplayedValue = useMemo(() => {
        if (isEditing) {
            ...
            const formatted = numericValue !== undefined && numericValue !== null && !isNaN(numericValue) ? formatValue(numericValue) : "";
            return formatted;
        }
