import React, { useState, forwardRef, useRef, useCallback, useId } from "react";
    ...
    (
        {
            ...
            errorMessage,
            helperText,
            placeholder = "Arrastra y suelta archivos aquí o haz clic para seleccionarlos",
            multiple = false,
            accept,
            onChange,
            onFocus,
        ...
        const internalRefCallback = useCallback((node: HTMLInputElement | null) => {
        ...
        // Helper para verificar si un archivo coincide con la cadena accept
        ...
        const handleFileChange = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
            ...
            if (multiple) {
                ...
                filesToProcess.forEach(file => {
                    const isDuplicate = newSelectedFilesData.some(item =>
                ...
                if (filesToProcess.length > 0) {
                    newSelectedFilesData = [{
                        ...
                        id: `${Date.now()}-${filesToProcess[0].name}-${Math.random().toString(36).slice(2, 9)}`,
                        previewUrl: undefined,
                    }];
                } else {
                    newSelectedFilesData = [];
        ...
        const handleDrop = useCallback((event: React.DragEvent<HTMLElement>) => {
            ...
            const syntheticChangeEvent = { target: { files: files, value: '' }, } as React.ChangeEvent<HTMLInputElement>;
        ...
        if (type === 'dropzone') {
            ...
            if (!hasFiles) {
                ...
                interactiveContentJSX = (
                    <div className={styles["lambda-file-upload-drop-zone-single-image-preview-container"]}>
                        {singleFileData?.previewUrl ? (
                            <img src={singleFileData.previewUrl} alt={`Preview of ${singleFileData.file.name}`} className={styles["lambda-file-upload-drop-zone-single-image-preview"]} />
                        ) : (
                            <ImageIcon className={styles["lambda-file-upload-drop-zone-single-image-preview-icon"]} />
                        )}
                ...
                interactiveContentJSX = (
                    <span className={styles["lambda-file-upload-drop-zone-file-count"]}>
                        {selectedFilesData.length} archivo{selectedFilesData.length > 1 ? 's' : ''} seleccionado{selectedFilesData.length > 1 ? 's' : ''}
                    </span>
                );
            }
            ...
            interactiveContentJSX = (
                <button
                    ref={buttonRef}
                    className={buttonFileUpload({ size, radius, disabled, invalid })}
        ...
        return (
            ...
            >
                ...
                {type === "dropzone" ? (
                    ...
                    <div className={clsx(styles["lambda-file-upload-button-layout-container"])}>
                                    ...
                                    {isSingleFile && isSingleImage ? (
                                        <>
                                            <div className={buttonFilePreview({ size })}>
                                                {singleFileData?.previewUrl ? (
                                                    <img src={singleFileData.previewUrl} alt={`Preview of ${singleFileData.file.name}`} className={styles["lambda-file-upload-button-file-preview-image"]} />
                                                ) : (
                                                    <ImageIcon className={styles["lambda-file-upload-button-file-preview-icon"]} />
                                                )}
                                            </div>
                                            ...
                                            <button type="button" onClick={() => singleFileData && handleRemoveFile(singleFileData.id)} className={styles["lambda-file-upload-file-name-remove-button"]} aria-label={`Remover archivo ${singleFileData?.file.name}`}>
                                                <XCircleIcon className={styles["lambda-file-upload-file-name-remove-icon"]} />
                                            </button>
                                        </>
                                    ) : isSingleFile && !isSingleImage ? (
                                        <>
                                        ...
                                        <span className={styles["lambda-file-upload-file-name-display"]}>
                                            {selectedFilesData.length} archivo{selectedFilesData.length > 1 ? 's' : ''}
                                        </span>
                        ...
                        {type === 'button' && hasFiles && multiple && (
                            <ul className={clsx(styles["lambda-file-upload-file-list"], fileList({ size, invalid }))} role="list">
                                {selectedFilesData.map(fileData => (
                                    <FileItem
                                        key={fileData.id}
                                        fileData={fileData}
                ...
                {helperText && !invalid && (
                    <HelperText id={helperId} text={helperText} size={size} disabled={disabled} focused={focused} />
                )}
